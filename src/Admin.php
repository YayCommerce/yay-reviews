...
class Admin {
    ...
    public function register_rest_api() {
        register_rest_route( YAY_REVIEWS_REST_URL, 'get-settings', [
            'methods' => 'GET',
            'callback' => array( $this, 'rest_get_settings' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);
        register_rest_route( YAY_REVIEWS_REST_URL, 'save-settings', [
            'methods' => 'POST',
            'callback' => array( $this, 'rest_save_settings' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);

        register_rest_route( YAY_REVIEWS_REST_URL, 'get-products', [
            'methods' => 'GET',
            'callback' => array( $this, 'rest_get_products' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);
        register_rest_route( YAY_REVIEWS_REST_URL, 'get-categories', [
            'methods' => 'GET',
            'callback' => array( $this, 'rest_get_categories' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);
        register_rest_route( YAY_REVIEWS_REST_URL, 'get-coupons', [
            'methods' => 'GET',
            'callback' => array( $this, 'rest_get_coupons' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);

        register_rest_route( YAY_REVIEWS_REST_URL, 'email-preview', [
            'methods' => 'POST',
            'callback' => array( $this, 'rest_email_preview' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);
        register_rest_route( YAY_REVIEWS_REST_URL, 'export-reviews', [
            'methods' => 'POST',
            'callback' => array( $this, 'rest_export_reviews' ),
            'permission_callback' => array( $this, 'rest_permission_callback' ),
        ]);
        register_rest_route( YAY_REVIEWS_REST_URL, 'import-reviews', [
            'methods' => 'POST',
    ...
    public function rest_save_settings(\WP_REST_Request $request) {
        ...
        if(!is_null($coupon) && is_array( $coupon ) ) {
            ...
            $coupon['coupons'] = array_map( function($c){
    ...
    public function rest_get_categories(\WP_REST_Request $request) {
        ...
        if (is_wp_error($categories)) {
            return rest_ensure_response(['error' => $categories->get_error_message()]);
        }
    ...
    Warm regards,",
            ...
            $table = Helpers::get_table_of_products( array('orderby' => 'date', 'order' => 'DESC') , 3, $data);
        ...
        return rest_ensure_response( array('success' => true, 'html' => $email_content) );
    }
    public function rest_export_reviews(\WP_REST_Request $request) {
        ...
        $date_range = isset($fields['date_range']) ? $fields['date_range'] : [];
        $products = isset($fields['products']) ? $fields['products'] : [];
        $categories = isset($fields['categories']) ? $fields['categories'] : [];
        $ratings = isset($fields['ratings']) ? array_map('intval', $fields['ratings']) : [];
        ...
        if (!empty($status)) {
            ...
            $status = array_map(function($s) use ($status_map) {
        ...
        foreach ($reviews as $review) {
            ...
            if (!empty($photos_meta) && is_array($photos_meta)) {
                $photos = implode("\n", array_map(function($path) use ($upload_baseurl) {
    ...
    public function admin_enqueue_scripts( $hook ) {
        if($hook == 'yaycommerce_page_yay-reviews') {
            ...
            $deps_n_ver = include_once YAY_REVIEWS_PLUGIN_PATH . 'assets/admin/dist/main.jsx.asset.php';
            wp_enqueue_style('yay-reviews', YAY_REVIEWS_PLUGIN_URL . 'assets/admin/dist/main.jsx.css', array(), $deps_n_ver['version']);

            wp_register_script(
                'yay-reviews',
                YAY_REVIEWS_PLUGIN_URL . 'assets/admin/dist/main.jsx.js',
                $deps_n_ver['dependencies'],
                $deps_n_ver['version'],
                true
            );
        ...
        wp_enqueue_script('yay-reviews-script', YAY_REVIEWS_PLUGIN_URL . 'assets/admin/js/yay-reviews.js', array('jquery'), '1.0');
    }
