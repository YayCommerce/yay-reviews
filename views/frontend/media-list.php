...
<div class="yayrev-review__media-list">
	<?php
	foreach ( $files as $key => $file ) :
		$extension  = pathinfo( $file, PATHINFO_EXTENSION );
		$media_type = in_array( strtolower( $extension ), BaseConstants::SUPPORTED_VIDEO_EXTENSIONS ) ? 'video' : 'image';
		?>
		<div class="yayrev-review__media-item yayrev-media-card" data-comment-id="<?php echo esc_attr( $comment->comment_ID ); ?>" data-index="<?php echo esc_attr( $key ); ?>" data-type="<?php echo esc_attr( $media_type ); ?>">
				...
				<div class="yayrev-media-card-inner">
					<?php if ( 'video' === $media_type ) : ?>
						...
						/* @codingStandardsIgnoreStart */
						 ?>
						<img class="yayrev-media-card__video-thumbnail yayrev-video_thumbnail" src="<?php echo esc_url( YAYREV_PLUGIN_URL . 'assets/frontend/img/video-thumbnail.png' ); ?>" data-src="<?php echo esc_url( $uploads['baseurl'] . $file ); ?>" alt="video_thumbnail" />
...
<div class="yayrev-review-details-modal" data-comment-id="<?php echo esc_attr( $comment->comment_ID ); ?>">
	<div class="yayrev-modal-content">
		<div class="yayrev-modal-comment-details">
			<div class="yayrev-modal-header">
				<h1><?php echo esc_html__( 'Review Details', 'yay-customer-reviews-woocommerce' ); ?></h1>
				<?php if ( ! is_admin() ) : ?>
					<span class="yayrev-modal-see-all-media"><?php echo esc_html__( 'See all media', 'yay-customer-reviews-woocommerce' ); ?></span>
				<?php endif; ?>
			</div>
		</div>
	</div>
	...
	?>
	<div class="yayrev-modal-media-frame-content">
		<div class="yayrev-modal-media-frame-content-left">
			<?php if ( $has_multiple_media ) : ?>
				<div class="yayrev-nav-arrow yayrev-nav-prev" aria-label="Previous media">
			...
			<?php if ( $has_multiple_media ) : ?>
				<div class="yayrev-nav-arrow yayrev-nav-next" aria-label="Next media">
		...
		<div class="yayrev-modal-media-frame-content-right">
				...
				<div class="comment-info">
					<div class="comment-author-avatar">
						...
						<?php if ( 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) && wc_review_is_from_verified_owner( $comment->comment_ID ) ) : ?>
							<span class="yayrev-verified-badge" data-tippy-content="<?php esc_attr_e( 'Verified owner', 'yay-customer-reviews-woocommerce' ); ?>"><svg xmlns="http://www.w3.org/2000/svg " width="24" height="24" viewBox="0 0 24 24" fill="#067D62">
					...
					?>
					<div class="yayrev-modal-comment-rating"><?php echo wp_kses_post( $rating_stars ); ?></div> 
					<?php
						$comment_date = get_comment_date( 'F j, Y', $comment );
						$comment_time = get_comment_date( 'g:i A', $comment );
					?>
					<div class="comment-date"><?php echo esc_html__( 'Reviewed in ', 'yay-customer-reviews-woocommerce' ) . ' ' . esc_html( $comment_date ) . ' ' . esc_html__( 'at', 'yay-customer-reviews-woocommerce' ) . ' ' . esc_html( $comment_time ); ?></div>
				</div>
				<div class="comment-content"><?php echo wp_kses_post( $comment->comment_content ); ?></div>
				<div class="yayrev-review-details__medias-wrapper">
					<div class="yayrev-review-details__medias-title"><?php echo esc_html__( 'Medias in this review', 'yay-customer-reviews-woocommerce' ); ?></div>
					<div class="yayrev-review-details__media-list">
						<?php foreach ( $files as $key => $file ) : ?>
							...
							?>
							<?php if ( 'video' === $media_type ) : ?>
							<div class="yayrev-modal-comment-medias-preview-item yayrev-media-card" data-index="<?php echo esc_attr( $key ); ?>" data-type="video">
								<div class="yayrev-media-card-wrapper">
									<div class="yayrev-media-card-inner">
										<?php
										/* @codingStandardsIgnoreStart */
										 ?>
										<img class="yayrev-video_thumbnail" src="<?php echo esc_url( YAYREV_PLUGIN_URL . 'assets/frontend/img/video-thumbnail.png' ); ?>" alt="Media preview" data-src="<?php echo esc_url( $uploads['baseurl'] . $file ); ?>" />
										<?php /* @codingStandardsIgnoreEnd */ ?>
										<div class="yayrev-media-card__video-details"></div>
									</div>
								</div>
							...
							<?php else : ?>
							<div class="yayrev-modal-comment-medias-preview-item yayrev-media-card" data-index="<?php echo esc_attr( $key ); ?>" data-type="image">
								<div class="yayrev-media-card-wrapper">
									<div class="yayrev-media-card-inner">
										<?php
										/* @codingStandardsIgnoreStart */
...
<div class="yayrev-modal-backdrop" data-comment-id="<?php echo esc_attr( $comment->comment_ID ); ?>"></div>
